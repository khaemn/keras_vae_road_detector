...
_INPUT_DIR = '/home/rattus/Projects/PythonNN/datasets/downloaded-assorted/genmasks/'
...
def getVerticalMiddleSplittingLine(mask):
    ...
    height_step, bottom_offset = divmod(height, total_h_steps)

    lefts, centers, rights = [], [], []

    for step in range(0, total_h_steps):
        y = bottom_offset + step * height_step
        prev_pix = mask[y, 0]
        ...
        for index in range(1, width):
            pix = mask[y, index]
            if pix > prev_pix or (pix == prev_pix and index == 0):
                if (left_ind < 0):
                    lefts.append([index, y])
                    left_ind = index
            if pix < prev_pix or (right_ind < 0 and index == width - 1):
                if (right_ind < 0):
